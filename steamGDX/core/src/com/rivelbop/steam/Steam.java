...
public class Steam {
	...
	private static enum Callback {
		USER, FRIENDS, MATCHMAKING, NETWORKING;
	}

	// Predetermined channels to send data over
	public static class Channel {
		public static final int MESSAGE = 0, VOICE = 1;
	}
	...
	public static class User {
		...
		public static ByteBuffer getVoice(ByteBuffer buffer) {
			...
			try {
				VoiceResult result = user.decompressVoice(buffer, audio, bytesWritten,
	...
	public static class Lobby {
		...
		public static void sendMessage(String message) {
			if (inLobby())
				matchmaking.sendLobbyChatMsg(getID(), message);
		}
		...
		public static SteamID getID() {
			return checkIfDefault(Callback.MATCHMAKING) ? ((DefaultMatchmakingCallback) matchmakingCallback).lobbyID
					: null;
		}

		// Check to see if the player is in a lobby
		public static boolean inLobby() {
			return inLobby && (getID() != null || !checkIfDefault(Callback.MATCHMAKING));
		}

		// Return the messages sent in the lobby
		public static ArrayList<LobbyMessage> getMessages() {
			return checkIfDefault(Callback.MATCHMAKING) ? ((DefaultMatchmakingCallback) matchmakingCallback).messages
					: null;
		}
		...
		public static ArrayList<SteamID> players() {
			if (inLobby()) {
				ArrayList<SteamID> players = new ArrayList<>();
				for (int i = 0; i < count(); i++)
	...
	public static class Network {
		...
		public static PacketData receivePacket(int channel) {
			...
			if (networking.isP2PPacketAvailable(channel, packetSize)) {
				...
				ByteBuffer buffer = (channel != Channel.VOICE) ? ByteBuffer.allocateDirect(BUFFERSIZE)
						: ByteBuffer.allocateDirect(VOICEBUFFERSIZE);
				...
				if (packetReadSize > 0) {
					...
					System.out.println(
							"Packet recieved by: " + Friends.getUsername(player) + ", " + bytesReceived + " bytes");
					...
					case Channel.VOICE:
						return new PacketData(player, Steam.User.getVoice(buffer));
					default:
